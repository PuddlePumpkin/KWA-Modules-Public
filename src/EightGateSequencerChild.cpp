...
struct EightGateSequencerChild : Module {
	...
	void process(const ProcessArgs& args) override {
		...
		// Clock Schmitt
		if (clockInputSchmitt.process(expanderInputClock, 0.1f, 1.f)) {
		...
		// Reset Schmitt 
		if (resetInputSchmitt.process(expanderInputReset, 0.1f, 1.f)) {
			activeStep = (expanderInputContinuousMode ? -1 : 0);
		...
		if (is_momma) {
			...
			//[0] - Clock
			rightMessage[0] = expanderInputClock;
			//[1] - Reset
			rightMessage[1] = expanderInputReset;
			//[2] - Continuous mode
